...
const Review = () => {
  ...
  return (
    <div className="mt-11">
      <Heading5 className="font-poppins text-[#424242] mb-7">Customer Review</Heading5>
        ...
        <div className={`flex flex-row items-center justify-start ${idx !== 0 && 'mt-10'}`} key={item.id}>
          <div className="flex flex-col justify-center items-start w-[54%]">
            <div className="flex flex-row items-center justify-start">
              ...
              <Body2 className="text-[#7F7F7F] ml-2">{item.name}</Body2>
            </div>
            <div className="flex flex-row items-center justify-start mt-2">
              <div className='flex flex-row items-center justify-start gap-1'>
                {Array(5).fill({}).map((arr, idx) => idx + 1 <= item.star ? star(true, idx) : star(false, idx))}
              </div>
              <Body2 className="text-[#424242] ml-2 mt-2">Awesome Hoodie!</Body2>
            </div>
            <Caption className="text-[#91929D] mt-2">{item.date}</Caption>
            <SubTitle1 className="text-[#424242] leading-8">{item.description}</SubTitle1>
          </div>
          <div className="w-[15%] ml-[1%]">
            <Image src={hoodie} alt="product" className="w-full h-fit" />
          </div>
          <div className='flex flex-row items-center w-[17%] ml-[10%]'>
            <div className='flex flex-row items-center'>
              ...
              <Body2 className="text-[#000000] ml-2.5">Helpful ({item.like})</Body2>
            </div>
            <Body2 className="text-[#7F7F7F] ml-[10%]">Report</Body2>
          </div>
        </div>
      )}
      <Pagination
