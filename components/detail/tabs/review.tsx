...
const Review = ({data}: {data: IComment[]}) => {
  ...
  return (
    <div className="mt-11">
      <Heading6 className="font-poppins text-light-200 mb-7">Customer Review</Heading6>
      {data?.filter((comment, idx) => idx >= (page * 3) - 3 && idx <= page * 3).map((item: IComment, idx: number) =>
        <div className={`flex flex-row items-center justify-between ${idx !== 0 && 'mt-10'}`} key={item.id}>
          ...
          <div className='flex flex-col gap-4 items-center w-[17%] ml-[10%]'>
            <div className='flex flex-row items-center'>
              ...
              <Body2 className="text-dark-500 ml-2.5">Helpful ({item.like})</Body2>
            </div>
