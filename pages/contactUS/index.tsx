...
const index = () => {
  ...
  return (
    <Layout title='Contact Us'>
      <div className="container mx-auto flex flex-col md:flex-row items-center md:items-start justify-evenly gap-8 md:gap-4 mt-4 md:mt-24">
        <div className="flex flex-col items-start w-3/4 md:w-2/5">
          <Heading4 className="them-contact-title mt-4 md:mt-12 mb-6 md:mb-20 cursor-default max-[400px]:text-2xl">How Can I Help You?</Heading4>
          <div className="flex flex-row items-center justify-start">
            ...
            <Tooltip title={copy.phone ? 'copied ✔' : 'Click to copy'} onMouseLeave={() => setCopy({ phone: false, email: false })}>
              <Heading6
                className="text-light-100 ml-2 cursor-copy max-[400px]:text-base"
                onClick={e => {
                  navigator.clipboard.writeText('0123456789')
                  setCopy({ email: false, phone: true })
                }}
              >+98 0123456789</Heading6>
            </Tooltip>
          </div>
          <div className="flex flex-row items-center justify-start mt-4 md:mt-8">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <Tooltip title={copy.email ? 'copied ✔' : 'Click to copy'} onMouseLeave={() => setCopy({ phone: false, email: false })}>
              <Heading6
                className="text-light-100 ml-2 cursor-copy max-[400px]:text-base"
                onClick={e => {
                  navigator.clipboard.writeText('example@gmail.com')
                  setCopy({ phone: false, email: true })
                }}
              >example@gmail.com</Heading6>
            </Tooltip>
          </div>
        </div>
        <ContactUs />
